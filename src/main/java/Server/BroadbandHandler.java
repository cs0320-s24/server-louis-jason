...
//    Set<String> params = request.queryParams();
    ...
    try {
      ...
      List<CountyNumberResponse> countyNumberList = deserializeCountyNumber(sendCountyRequest(stateCode));
      String countyCode = "*";
...
//    System.out.println(sentCensusResponse.body());
  ...
  private String sendCountyRequest(String state)
    ...
    HttpRequest retrieveCountyNums =
            HttpRequest.newBuilder()
                    .uri(new URI("https://api.census.gov/data/2010/dec/sf1?get=NAME&for=county:*&in=state:" + state))
                    .GET()
                    .build();
...
//    System.out.println(sentBoredApiResponse.body());
    HashMap<String, String> stateCodesMap = BroadbandAPIUtilities.deserializeBroadband(stateNums.body());
    return stateCodesMap;
  }

  public static List<CountyNumberResponse> deserializeCountyNumber(String jsonList) throws IOException {
